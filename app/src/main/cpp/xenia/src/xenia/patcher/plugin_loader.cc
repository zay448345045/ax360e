...
void PluginLoader::LoadTitleConfig(const uint32_t title_id) {
  ...
  try {
    plugins_config = cpptoml::parse_file(title_plugins_config.string());
  } catch (...) {
  ...
  const auto plugin_array = plugins_config->get_array("plugin");
  if (!plugin_array) {
    return;
  }
  ...
  for (const auto& plugin_entry : *plugin_array) {
    ...
    if (plugin_entry->as_table()->contains("name")) {
      entry.name = *plugin_entry->as_table()->get_as<std::string>("name");
    }
    if (plugin_entry->as_table()->contains("file")) {
      entry.file = xe::string_util::trim(
          *plugin_entry->as_table()->get_as<std::string>("file"));
    }
    if (plugin_entry->as_table()->contains("desc")) {
      entry.desc = *plugin_entry->as_table()->get_as<std::string>("desc");
    }
    if (plugin_entry->as_table()->contains("is_enabled")) {
      entry.is_enabled =
          *plugin_entry->as_table()->get_as<bool>("is_enabled");
    }
...
std::vector<uint64_t> PluginLoader::GetHashes(
        const std::shared_ptr<cpptoml::base>& toml_entry) const {
  ...
  if (toml_entry->is_array()) {
    for (const auto& hash_entry : *toml_entry->as_array()) {
      if (hash_entry->as<std::string>()->get().empty()) {
        continue;
      }

      hashes.push_back(xe::string_util::from_string<uint64_t>(
          hash_entry->as<std::string>()->get(), true));
    }
  }
  return hashes;
}
