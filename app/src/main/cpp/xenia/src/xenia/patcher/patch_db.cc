...
PatchFileEntry PatchDB::ReadPatchFile(
  ...
  try {
    patch_toml_fields =cpptoml::parse_file(file_path.string());
  } catch (...) {
...
bool PatchDB::ReadPatchData(std::vector<PatchDataEntry>& patch_data,
                            ...
                            const std::shared_ptr<cpptoml::table>& patch_fields) const {
  auto patch_data_fields = patch_fields->get_array(data_type.first);
  ...
  for (const auto& patch_data_table : *patch_data_fields) {
    ...
    switch (data_type.second.type) {
      ...
      case PatchDataType::kBE32: {
        uint32_t value =
            static_cast<uint32_t>(*table->get_as<int64_t>("value"));
        patch_data.push_back(
            {address, PatchDataValue(alloc_size, xe::byte_swap(value))});
        break;
      }
      case PatchDataType::kBE64: {
        uint64_t value = *table->get_as<int64_t>("value");
        patch_data.push_back(
            {address, PatchDataValue(alloc_size, xe::byte_swap(value))});
        break;
      }
      case PatchDataType::kF64: {
        ...
        double value = 0.0;
        if (value_field->as<double>()) {
          value = value_field->as<double>()->get();
        }
        if (value_field->as<int64_t>()) {
          value = static_cast<double>(value_field->as<int64_t>()->get());
        }
      ...
      case PatchDataType::kF32: {
        ...
        float value = 0.0f;
        if (value_field->as<double>()) {
          value = static_cast<float>(value_field->as<double>()->get());
        }
        if (value_field->as<int64_t>()) {
          value = static_cast<float>(value_field->as<int64_t>()->get());
        }
      ...
      case PatchDataType::kString: {
        std::string value = *table->get_as<std::string>("value");
        patch_data.push_back({address, PatchDataValue(value)});
        break;
      }
      case PatchDataType::kU16String: {
        std::u16string value =
            xe::to_utf16(*table->get_as<std::string>("value"));
        patch_data.push_back({address, PatchDataValue(value)});
        break;
      }
      case PatchDataType::kByteArray: {
        std::vector<uint8_t> data;
        const std::string value = *table->get_as<std::string>("value");
...
void PatchDB::ReadHashes(PatchFileEntry& patch_entry,
                         const std::shared_ptr<cpptoml::base>& hashes_node) const {
  auto add_hash = [&patch_entry](const std::shared_ptr<cpptoml::base>& hash_node) {
    if (!hash_node->as<std::string>()) {
      return;
    }

    const auto string_hash = hash_node->as<std::string>()->get();
...
void PatchDB::ReadPatchHeader(PatchInfoEntry& patch_info,
  ...
  if (patch_fields->contains("name")) {
      if(const auto patch_name_field = patch_fields->get_as<std::string>("name"))
    patch_name = *patch_name_field;
  }
  if (patch_fields->contains("desc")) {
    if(const auto patch_desc_field = patch_fields->get_as<std::string>("desc"))
        patch_desc = *patch_desc_field;
  }
  if (patch_fields->contains("author")) {
    if(const auto patch_author_field = patch_fields->get_as<std::string>("author"))
        patch_author = *patch_author_field;
  }
  if (patch_fields->contains("is_enabled")) {
    if(const auto patch_is_enabled_field = patch_fields->get_as<bool>("is_enabled"))
        is_enabled = *patch_is_enabled_field;
  }
