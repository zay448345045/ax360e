...
#define TARGET_THREAD 24

std::atomic<uint64_t> trace_index;
constexpr uint64_t TRACE_START = 2150'0000;//2500'0000;

#define TRACE_CHECK()                       \
  if (thread_state->thread_id() != TARGET_THREAD) return; \
  if (++trace_index < TRACE_START) return; \
  XELOGI("{}", trace_index);
//#define TRACE_CHECK() return;

#define TRACE_RETURN() return;
/*
uint64_t trace_indexs[64]={0};
#define TRACE_CHECK()                       \
  if (++trace_indexs[thread_state->thread_id()]%500'0000==0)\
  XELOGI("{}: {}",thread_state->thread_id(), trace_indexs[thread_state->thread_id()]); return;
*/
...
#define DPRINT(...)                  \
  if (trace_enabled && THREAD_MATCH) \
  xe::logging::AppendLogLineFormat(xe::LogSrc::Cpu, xe::LogLevel::Info, 't', __VA_ARGS__)
