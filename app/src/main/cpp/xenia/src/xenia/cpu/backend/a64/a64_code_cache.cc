...
void A64CodeCache::CommitExecutableRange(uint32_t guest_low,
  ...
  auto indirection_table_base=indirection_table_base_;

  // Commit the memory.
  xe::memory::AllocFixed(
      indirection_table_base + (guest_low - kIndirectionTableBaseLow),
      guest_high - guest_low, xe::memory::AllocationType::kCommit,
      xe::memory::PageAccess::kReadWrite);

  // Fill memory with the default value.
  uint32_t* p = reinterpret_cast<uint32_t*>(indirection_table_base);
  ...
  UnwindReservation unwind_reservation;
  {
    ...
    std::memset(tail_write_address, 0x00,
      ...
      {//fix opcodes
          ...
          constexpr uint32_t movk_xn_imm16_lsl_32 = 0xf2c00000;
          const uint32_t imm16=execute_address_high()>>32;
          assert(imm16>=1&&imm16<=127);
          const uint32_t movk_x16_$imm16_lsl_32 = movk_xn_imm16_lsl_32 | 16 | (imm16 << 5);
          const uint32_t movk_x17_$imm16_lsl_32 = movk_xn_imm16_lsl_32 | 17 | (imm16 << 5);
#if 0
          for (const auto&  fix: fix_opcodes_) {
              if (fix.inst_offset() >=func_info.code_size.total) {
                  continue;
              }
              if(*reinterpret_cast<uint32_t *>(code_write_address + fix.inst_offset()) != nop){
                  XELOGE("A64CodeCache::PlaceGuestCode: fix opcode error");
                  continue;
              }
              if (fix.type() == InstFix::TY_MOVK_X16_IMM16_SHIFT32) {
                  *reinterpret_cast<uint32_t *>(code_write_address + fix.inst_offset()) = movk_x16_$0xa_lsl_32;
              } else if (fix.type() == InstFix::TY_MOVK_X17_IMM16_SHIFT32) {
                  *reinterpret_cast<uint32_t *>(code_write_address + fix.inst_offset()) = movk_x17_$0xa_lsl_32;
              }
          }
#else
          bool fix_movk_x17=true;
          uint32_t* code_ptr=reinterpret_cast<uint32_t*>(code_write_address);
          for(int i=0;i<func_info.code_size.total/4;i++) {
              if(code_ptr[i]==nop){
                  code_ptr[i]=fix_movk_x17?movk_x17_$imm16_lsl_32:movk_x16_$imm16_lsl_32;
                  fix_movk_x17=!fix_movk_x17;
              }
          }
#endif
      }
