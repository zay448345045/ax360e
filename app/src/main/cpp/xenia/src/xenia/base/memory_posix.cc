...
void* AllocFixed(void* base_address, size_t length,
  ...
  // mmap does not support reserve / commit, so ignore allocation_type.
    ...
    //XELOGI("AllocFixed: 0x{:X} 0x{:X} ", (uintptr_t)base_address, length);
...
bool DeallocFixed(void* base_address, size_t length,
    ...
    //XELOGI("DeallocFixed: 0x{:X} 0x{:X} ", (uintptr_t)base_address, length);
...
void* MapFileView(FileMappingHandle handle, void* base_address, size_t length,
  ...
  uint32_t prot = ToPosixProtectFlags(access);
    //XELOGI("MapFileView: 0x{:X} 0x{:X} {}", (uintptr_t)base_address, length,handle);
...
bool UnmapFileView(FileMappingHandle handle, void* base_address,
    ...
    //XELOGI("UnmapFileView: 0x{:X} 0x{:X} ", (uintptr_t)base_address, length);
    std::lock_guard guard(g_mapped_file_ranges_mutex);
